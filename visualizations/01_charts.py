...
with plt.style.context('dark_background'):
    ...
    for ax, series_type, title, color in [
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([SERIES_LABELS[s] for s in series_ids], fontsize=9, color='white')
        ax.set_title(title, fontsize=11, fontweight='bold', color='white')
        ax.set_ylabel('Avg Goal Diff (trailing team)' if ax == axes[0] else '', color='white')
        ax.legend(fontsize=8, facecolor='#222222', edgecolor='#555555', labelcolor='white')
        ax.set_ylim(-5, 6)
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'chart1_goal_differential.png', dpi=150, bbox_inches='tight', facecolor='#111111')
    plt.close()
...
with plt.style.context('dark_background'):
    ...
    for ax, sid in zip(axes.flat, comeback_series):
        ...
        ax.set_ylabel('Cumulative goal differential', fontsize=9, color='white')
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'chart2_cumulative_goal_diff.png', dpi=150, bbox_inches='tight', facecolor='#111111')
    plt.close()
...
fig, ax = plt.subplots(figsize=(11, 5), facecolor='#111111')
ax.set_facecolor('#111111')
...
ax.axhline(0, color='#888888', linewidth=0.8, linestyle='--')
ax.set_xticks(x)
ax.set_xticklabels([SERIES_LABELS[s] for s in all_series], fontsize=9, color='white')
ax.set_ylabel('Simple Rating System (SRS)', fontsize=10, color='white')
ax.set_xlabel('← Comeback Series  |  Comparison Series →', fontsize=9, labelpad=8, color='white')
...
plt.tight_layout()
plt.savefig(OUT_DIR / 'chart3_srs_comparison.png', dpi=150, bbox_inches='tight', facecolor='#111111')
plt.close()
...
with plt.style.context('dark_background'):
    ...
    for ax, sid in zip(axes.flat, comeback_series):
        ...
        ax.set_title(FULL_NAMES[sid], fontsize=10, fontweight='bold', color='white')
        ax.set_xlabel('Game number', fontsize=9, color='white')
        ax.set_ylabel('Cumulative wins', fontsize=9, color='white')
        ax.set_xticks(s['game_num'].tolist())
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'chart4_series_progression.png', dpi=150, bbox_inches='tight', facecolor='#111111')
    plt.close()
